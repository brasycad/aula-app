...
export class Watch {
  ...
  @Input('external-color') externalColor: string
  @Input('inner-color') innerColor: string
  ...
  ngAfterViewInit() {
    ...
    this.interval = setInterval(() => this.setTime(this.isSpinner ? new Date(random(1000, 1000000000)) : new Date()), 1000);
  }
  setTime(time: Date) {
    ...
    if (this.isSpinner) {
      const ele: any = document.querySelector('.component-wrapper');
